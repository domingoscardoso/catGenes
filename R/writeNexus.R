...
#' writing of the NEXUS file will not be printed in the console in full.
#'
...
writeNexus <- function(x, file,
  ...
  writtenby <- paste("[catGenes (JBRJ), ", date(), "]\n\n", sep = "")
  ...
  if (verbose) {
    message("The final dataset will have these DIMENSIONS:\n",
            nchartotal, "\n", ntax, "\n")
  }
  ...
  if (interleave) {
    if (verbose) {
      message("Combining genes as interleave...\n")
    }
    ...
    if (verbose) {
      message("Concatenating genes in full, non-interleaved alignment...\n")
    }
